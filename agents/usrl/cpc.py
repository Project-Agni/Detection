...
class CPCTrainer(Trainer):
    ...
    def __init__(self, encoder, config, device=torch.device("cpu"), wandb=None):
        ...
        self.device = device
        self.steps_gen = lambda: range(
            self.steps_start, self.steps_end, self.steps_step
        )
        self.discriminators = {
            i: nn.Linear(self.gru_size, self.encoder.hidden_size).to(device)
        ...
        self.labels = {
            i: torch.arange(self.batch_size * (self.sequence_length - i - 1)).to(device)
            for i in self.steps_gen()
        }
    ...
    def generate_batch(self, episodes):
        episodes = [
            episode for episode in episodes if len(episode) >= self.sequence_length
        ]
        # Episode sampler
        ...
        sampler = BatchSampler(
            RandomSampler(
                range(len(episodes)),
                replacement=True,
                num_samples=len(episodes) * self.sequence_length,
            ),
            self.batch_size,
            drop_last=True,
        )
        for indices in sampler:
            ...
            for episode in episodes_batch:
                start_index = np.random.randint(
                    0, len(episode) - self.sequence_length + 1
                )
    ...
    def do_one_epoch(self, epoch, episodes):
        ...
        for sequence in data_generator:
            with torch.set_grad_enabled(mode == "train"):
                ...
                for i in self.steps_gen():
                    predictions = (
                        self.discriminators[i](contexts[:, : -(i + 1), :])
                        .contiguous()
                        .view(-1, self.encoder.hidden_size)
                    )
                    targets = (
                        latents[:, i + 1 :, :]
                        .contiguous()
                    ...
                    step_accuracy = (
                        preds.eq(self.labels[i]).sum().float() / self.labels[i].numel()
                    )
    ...
    def log_results(self, epoch_idx, epoch_losses, epoch_accuracies, prefix=""):
        ...
        for i in self.steps_gen():
            ...
            log_results[prefix + "_step_accuracy_{}".format(i + 1)] = epoch_accuracies[
                i
            ]
