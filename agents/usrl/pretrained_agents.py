...
def get_pretrained_rl_representations(args, steps):
    ...
    episodes, episode_labels, mean_reward = get_ppo_representations(
        args, steps, checkpoint
    )
...
def evaluate(
    actor_critic, env_name, seed, num_processes, eval_log_dir, device, num_evals
):
    eval_envs = mve(
        env_name,
        seed + num_processes,
    ...
    for step in range(steps // args.num_processes):
        ...
        if args.probe_collect_mode == "random_agent":
            action = torch.tensor(
                np.array(
                    [
                        np.random.randint(1, envs.action_space.n)
...
if __name__ == "__main__":
    ...
    episode_features, episode_labels, mean_reward = get_ppo_representations(
        args, 10753536
    )
