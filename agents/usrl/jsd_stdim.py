...
class SpatioTemporalTrainer(Trainer):
    def __init__(self, encoder, config, device=torch.device("cpu"), wandb=None):
        ...
        self.classifier1 = Classifier(
            self.encoder.hidden_size, self.encoder.local_layer_depth
        ).to(device)
    ...
    def generate_batch(self, episodes):
        ...
        for indices in sampler:
            ...
            for episode in episodes_batch:
                ...
                ts.append([t])
                thats.append([t_hat])
            yield torch.stack(x_t).float().to(self.device) / 255.0, torch.stack(
                x_tprev
            ).float().to(self.device) / 255.0, torch.stack(x_that).float().to(
                self.device
            ) / 255.0, torch.Tensor(
                ts
            ).to(
    ...
    def do_one_epoch(self, epoch, episodes):
        ...
        for x_t, x_tprev, x_that, ts, thats in data_generator:
            f_t_maps, f_t_prev_maps = self.encoder(x_t, fmaps=True), self.encoder(
                x_tprev, fmaps=True
            )
            ...
            f_t = (
                ...
                .unsqueeze(1)
                .expand(
                    -1, f_t_prev.size(1), f_t_prev.size(2), self.encoder.hidden_size
                )
            )

            target = torch.cat(
                (torch.ones_like(f_t[:, :, :, 0]), torch.zeros_like(f_t[:, :, :, 0])),
                dim=0,
            ).to(self.device)
            ...
            f_t = f_t_maps["f5"]
            x1_p, x2_p = torch.cat([f_t, f_t], dim=0), torch.cat(
                [f_t_prev, f_t_hat], dim=0
            )
    ...
    def log_results(
        ...
        print(
            "{} Epoch: {}, Epoch Loss: {}, {} Accuracy: {}".format(
                prefix.capitalize(),
                epoch_idx,
                epoch_loss,
                prefix.capitalize(),
        ...
        self.wandb.log(
            {
                ...
                prefix + "_accuracy1": accuracy1,
                prefix + "_loss2": epoch_loss2,
                prefix + "_accuracy2": accuracy2,
            },
            step=epoch_idx,
        )
